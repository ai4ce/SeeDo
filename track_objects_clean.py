...
def contour_painter(input_image, input_mask, mask_color=5, mask_alpha=0.7, contour_color=1, contour_width=3, ann_obj_id=None):
    assert input_image.shape[:2] == input_mask.shape, 'Different shapes between image and mask'
...
def write_video(frames, output_path, fps):
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Select encoder
...
def generate_dynamic_plots(mask_add, mask_min, num_frames):
    ...
    for frame_index in tqdm(range(num_frames)):
        for index, k in enumerate(mask_add.keys()):
            ...
            axs[index * 2].axvline(x=frame_index, color='r')  # Dynamic position

            axs[index * 2 + 1].clear()
            axs[index * 2 + 1].plot(mask_min[k], label=f'{k} min')
            axs[index * 2 + 1].axvline(x=frame_index, color='r')  # Dynamic position
...
def main(input_video_path, output_video_path, key_frames, bbx_file):
    ...
    # image=load_image_from_array(frames[0])
    # Change frames[0] into your source image, a numpy array 
    image_source, best_boxes, best_phrases, best_logits = bbx_extract(groundingdino_model, DEVICE, frames[0], obj_list)
    # frames[0] is a RGB picture
    # cv2.imwrite("./test_image.png", frames[0])
    ...
    # video_dir = os.path.dirname(input_video_path) + f'/sample_freq_{sample_freq}_' + input_video_path.split('/')[-1].split('.')[0]
    # if not os.path.exists(video_dir):
    #     video2jpg(input_video_path, video_dir, sample_freq)
    ...
    # video_dir = os.path.dirname(input_video_path) + '/' + input_video_path.split('/')[-1].split('.')[0]
    # if not os.path.exists(video_dir):
    #     video2jpg(input_video_path, video_dir, 1)
    ...
    # # Third Part: compute the center coordinates of masks of different objects in the selected key frames
    ...
    # # Fourth Part: append the painted frames into an output video
    # painted_frames = []
